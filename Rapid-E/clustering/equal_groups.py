...
def _kmeans_single(X, n_clusters, x_squared_norms, max_iter=300,
    ...
    for i in range(max_iter):
        ...
        if sp.issparse(X):
            centers = KMeans._centers_sparse(X, sample_weight, labels, n_clusters,
                                               distances)
        else:
            centers = KMeans._centers_dense(X, sample_weight, labels, n_clusters, distances)
...
def _labels_inertia(X, x_squared_norms, centers,
    ...
    if sp.issparse(X):
        inertia = k_means_.KMeans._assign_labels_csr(
            X, x_squared_norms, centers, labels, distances=distances)
    else:
        if precompute_distances:
            return _labels_inertia_precompute_dense(X, x_squared_norms,
                                                    centers, distances)
        inertia = k_means_.KMeans._assign_labels_array(
            X, x_squared_norms, centers, labels, distances=distances)
